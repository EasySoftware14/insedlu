    ...
    public partial class ActiveProjects : Page
    {
        ...
        protected void datagridview_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "ViewDetails")
            {
                ...
                var stats = GetStatus(projects.status);
                ...
                lblstore.Text = "Project Status : " + stats;
        ...
        private void EditProject(object sender, GridViewCommandEventArgs e)
        {
            ...
            var costPlan = _projectService.GetCostPlan(id).deliverable;
            var project = _projectService.GetProjectById(id);
            var riskAnalsis = _projectService.RiskAnalysis(id).risk_analysis;
            var bee = _projectService.GetProjectBeeStatus(id).content;
            //var reference = _projectService.GetProjectReference(id).content;
            var implement = _projectService.GetProjectImplementationTime(id).content;
            var why = _projectService.GetProjectWhyChooseBiz(id).content;
            var scope = _projectService.GetProjectScopeOfWork(id).content;
            var jvComp = _projectService.GetProjectJvCompany(id).content;
            var coverpage = _projectService.GetProjectCoverPage(id).content;

            //Session["Reference"] = reference;
            Session["Implement"] = implement;
            Session["Why"] = why;
            Session["scope"] = scope;
            Session["JVCompany"] = jvComp;
            Session["BEE"] = bee;
            Session["Coverpage"] = coverpage;
            Session["ProType"] = project.project_type;
            ...
            Session["RiskAnalysis"] = riskAnalsis;

            Session["ID"] = id;

            Response.Redirect("EditProject.aspx?id=" + id, true);
        ...
        private void ProvideContent(Project projects)
        {
            ...
            try
            {
                ...
                var c6 = new Chunk(company.background, courier);
                ...
                var c8 = new Chunk(costPlan.deliverable, courier);
